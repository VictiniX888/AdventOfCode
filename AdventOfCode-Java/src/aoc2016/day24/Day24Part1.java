...
public class Day24Part1 {
    ...
    public static void main(String[] args) {
        ...
        Pair<Integer, Integer> startPos = null;
        int points = 0;
    ...
    private static int bfs(String path, int steps, Set<Pair<Integer, Integer>> moves, Set<Pair<Integer, Integer>> visited, int[][] grid) {
        ...
        for (Pair<Integer, Integer> move : moves) {
            ...
            int x = move.getValue();
            if(grid[y+1][x] != -1) {
                if(grid[y+1][x] == 0 && !visited.contains(new Pair<>(y+1, x))) {
                    newMoves.add(new Pair<>(y+1, x));
                    visited.add(new Pair<>(y+1, x));
                }
                else if(grid[y+1][x] == nextPoint) {
                    if(path.length() > 1) {
                        newMoves.clear();
                        newMoves.add(new Pair<>(y+1, x));
                        visited.clear();
            ...
            if(grid[y][x+1] != -1) {
                if(grid[y][x+1] == 0 && !visited.contains(new Pair<>(y, x+1))) {
                    newMoves.add(new Pair<>(y, x+1));
                    visited.add(new Pair<>(y, x+1));
                }
                else if(grid[y][x+1] == nextPoint) {
                    if(path.length() > 1) {
                        newMoves.clear();
                        newMoves.add(new Pair<>(y, x+1));
                        visited.clear();
