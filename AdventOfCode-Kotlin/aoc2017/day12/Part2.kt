...
fun main(args: Array<String>) {
    ...
    if (input == null) {
        ...
        val output = getGroups(input.map { it.split(" <-> ", ", ").map { it.toInt() } }.toMap()).count()
        println(output)
    }
}

private fun getGroups(communications: Map<Int, List<Int>>): MutableSet<MutableSet<Int>> {

    val groups = mutableSetOf<MutableSet<Int>>()

    communications.forEach { t, _ ->
        if (groups.find { it.contains(t) } == null) {
            groups.add(getGroup(t, communications))
        }
    }
