    ...
    do {
        ...
        moves.forEach {
            ...
            when (it[0]) {
                ...
                'x' -> {
                    val (pos1, pos2) = it.drop(1).split('/').map { i -> i.toInt() }; order = exchange(pos1, pos2, order)
                }
                'p' -> {
                    val (p1, p2) = it.drop(1).toCharArray().filter { c -> c != '/' }; order = partner(p1, p2, order)
                }
            }
        }
    } while (!orders.contains(order))

    return orders[1_000_000_000 % orders.size]
}
...
private fun exchange(pos1: Int, pos2: Int, programs: String): String {
    ...
    if (pos1 > pos2) { max = pos1; min = pos2 } else if (pos1 < pos2) { max = pos2; min = pos1 } else { return programs }
